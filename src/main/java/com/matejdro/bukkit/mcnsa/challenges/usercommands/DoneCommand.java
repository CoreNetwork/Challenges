...
public class DoneCommand extends BaseUserCommand {
	...
	public Boolean run(CommandSender sender, String[] args) {
		...
		{
			try {
				PreparedStatement statement = IO.getConnection().prepareStatement("SELECT Level FROM weekly_levels WHERE Level > (SELECT IFNULL(MAX(Level), 0)FROM weekly_completed WHERE Player = ?) AND WeekID = ? ORDER BY Level ASC LIMIT 1");
