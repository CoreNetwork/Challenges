...
public class WorldEditHandler {
	 public static Block[] getWorldEditRegion(Player bplayer, boolean autoExpand)
	    {
	    	Plugin plugin = FlatcoreWeekly.instance.getServer().getPluginManager().getPlugin("WorldEdit");
			if (plugin != null)
			{
				...
				try {
					...
					{
						FlatcoreWeekly.log.log(Level.SEVERE, "[Jail] Error while retreiving WorldEdit region! - {0}", e.getMessage());
						e.printStackTrace();
					}
					return null;
				}
